...
public class purchasesFunc {

	private String uglob;

	public purchasesFunc(String uglob) {
		this.uglob = uglob;
	}
	...
	public String purchases() {
		...
		try {
			...
			statement = MySQLConnect_marketplace.conn.prepareStatement("SELECT product_boughtfrom, product_name, price, totalprice , photo, quantity, consignee, full_address, postal_code, phone_number, added_time FROM product_buy WHERE product_boughtby = ? ORDER BY added_time DESC");
			...
			System.out.println("row purchases: " + rowCount);
			...
			if (rowCount > 0) {
				...
				do {
					...
					xml += "<product_boughtfrom" + iterate_row + ">\n";
					xml += sqlResult.getString("product_boughtfrom")+"\n";
					xml += "</product_boughtfrom" + iterate_row + ">\n";
